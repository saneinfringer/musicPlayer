...
document.addEventListener("keyup", function (event) {
  ...
  if (event.key === "ArrowLeft") {
    ...
    nextSong();
  } else if ((event.key === " " || event.code === "Space") && !isInputFocused) {
    isPlayed ? pausemusic() : playmusic();
    renderPlaylist();
...
function renderPlaylist() {
  ...
  music.forEach((songObj, idx) => {
    const songItem = document.createElement("li");
    songItem.className = idx === musicIndex ? "active" : "";
    songItem.textContent = `${songObj.title} - ${songObj.Artist}`;
    songItem.dataset.index = idx;
...
// 2. Add song functionality
const addSongBtn = document.querySelector("#addSongBtn"); // Add a button in HTML with this id
addSongBtn.addEventListener("click", () => {
  const titleInput = document.querySelector("#songTitleInput").value;
  const artistInput = document.querySelector("#songArtistInput").value;
  const fileInput = document.querySelector("#songFileInput").files[0];
  const imageInput = document.querySelector("#songImageInput").value;

  if (titleInput && artistInput && fileInput && imageInput) {
    const fileURL = URL.createObjectURL(fileInput);
    music.push({
      name: fileInput.name,
      title: titleInput,
      Artist: artistInput,
      image: imageInput.replace(".jpg", ""),
      background: "#5A5A5A",
      fileURL: fileURL,
    });
    musicIndex = music.length - 1; // Set index to the newly added song
    loadSongs(music[musicIndex]); // Load the new song
